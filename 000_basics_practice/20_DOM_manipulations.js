...
(function() {
	...
	addBtn.addEventListener('click', ev => {
		...
		inputGroup.insertAdjacentHTML(
			'afterend',
			`<input type="text" class="form-control mb-2 mr-sm-2 mb-sm-0" id="inlineFormInput" placeholder="${++counter}">`
		);
	});
		...
		if (str) {
			...
			arr.forEach(item => {
				node.insertAdjacentHTML(
					'beforeend',
					`<input type="text" class="form-control mb-2 mr-sm-2 mb-sm-0" value=${item}>`
				);
			});
